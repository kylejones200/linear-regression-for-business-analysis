...
def fit_linear_regression(X: np.ndarray, y: np.ndarray) -> Tuple[LinearRegression, dict]:
    ...
    metrics = {'r2': r2_score(y, y_pred), 'rmse': np.sqrt(mean_squared_error(y, y_pred)), 'mae': mean_absolute_error(y, y_pred), 'coefficients': model.coef_.tolist(), 'intercept': model.intercept_}
    return (model, metrics)

def plot_regression_results(y_true: np.ndarray, y_pred: np.ndarray, title: str, output_path: Path, plot: bool=False):
